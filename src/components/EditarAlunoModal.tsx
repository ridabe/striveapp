...
// ─── Component ────────────────────────────────────────────────────────────────
export function EditarAlunoModal({ visible, student, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      birth_date: isoToMasked(student.birth_date),
      notes: student.notes ?? '',
    });
    const goal = student.goal ?? '';
    if (goal && STUDENT_GOALS.includes(goal)) {
      setGoalOption(goal);
      setGoalCustom('');
    } else if (goal) {
      setGoalOption('Outros');
      setGoalCustom(goal);
    } else {
      setGoalOption('');
      setGoalCustom('');
      ...
      >
        <View style={s.overlay}>
          <View style={s.sheet}>
            ...
            {/* Header */}
            <View style={s.header}>
              <Text style={s.title}>Editar Aluno</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={s.info}>
                ...
                <Text style={s.infoText}>
                  Se o email for alterado, o aluno passará a usar o novo endereço para fazer login.
                </Text>
              </View>
              ...
              >
                {saving ? (
                  ...
                  <>
                    <Ionicons name="checkmark-outline" size={18} color={primaryTextColor} />
                    <Text style={[s.submitText, { color: primaryTextColor }]}>Salvar alterações</Text>
                  </>
                )}
              </TouchableOpacity>
